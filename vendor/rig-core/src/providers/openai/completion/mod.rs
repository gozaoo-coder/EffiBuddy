...
pub struct Usage {
    ...
    // ===== EffiSuite vendored patch =====
    // DeepSeek 在 usage 顶层返回 prompt_cache_hit_tokens / prompt_cache_miss_tokens
    // （OpenAI 标准字段 prompt_tokens_details.cached_tokens 不返回）。
    // 补充解析这两个字段，供上层计费统计区分缓存命中/未命中。
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl GetTokenUsage for Usage {
    fn token_usage(&self) -> crate::completion::Usage {
        ...
        usage.reasoning_tokens = self
            ...
            .unwrap_or(0);
        // ===== EffiSuite vendored patch =====
        // DeepSeek 的缓存命中/未命中 token 数映射到核心 Usage 的
        // cached_input_tokens / cache_creation_input_tokens，供上层计费统计使用。
        if let Some(hit) = self.prompt_cache_hit_tokens {
            usage.cached_input_tokens = hit as u64;
        }
        if let Some(miss) = self.prompt_cache_miss_tokens {
            usage.cache_creation_input_tokens = miss as u64;
        }
        usage
    }
}
