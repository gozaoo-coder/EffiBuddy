...
impl Tool for SearchCodebaseTool {
    ...
    fn description(&self) -> String {
        ...
        format!(
            "用自然语言描述搜索代码库中的相关代码（语义搜索，非关键词精确匹配）。\
             输入一句自然语言描述（如「how does authentication work」「处理用户登录的逻辑」），\
             自动提取关键词后在工作区代码文件中检索最相关的代码块。\n\n\
             **与 search_file 的区别**：search_file 是关键词精确匹配（返回所有命中行）；\
             search_codebase 是语义相似度排序（返回 Top-N 最相关代码块），\
             适合「我想找做 X 的代码但不知道具体函数名」的场景。\n\n\
